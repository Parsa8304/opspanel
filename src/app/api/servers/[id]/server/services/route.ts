...
async function getServiceStatus(id: string, name: string) {
  try {
...
export const GET = handler(async (req: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
  ...
  const statuses = await Promise.all(
    KNOWN_SERVICES.map(async (svc) => {
      const status = await getServiceStatus(id, svc.name);
...
export const POST = handler(async (req: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
  ...
  const { stdout, stderr } = await targetExec(id, `systemctl ${action} ${service} 2>&1 || true`);
  await audit(
    u.id,
