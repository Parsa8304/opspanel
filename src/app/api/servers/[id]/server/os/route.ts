...
export const GET = handler(async (req: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
  ...
  const [osRelRes, kernelRes, uptimeRes, rebootRes, updatesRes, historyRes] =
    await Promise.allSettled([
      targetReadFile(id, "/etc/os-release"),
      targetExec(id, "uname -r 2>/dev/null"),
      targetExec(id, "uptime -p 2>/dev/null"),
      targetExec(id, "test -f /var/run/reboot-required && echo yes || echo no"),
      targetExec(id, "apt list --upgradable 2>/dev/null | tail -n +2 | head -50"),
      targetExec(id, "tail -50 /var/log/apt/history.log 2>/dev/null || echo ''"),
    ]);
  ...
  const arch = (
    await targetExec(id, "uname -m 2>/dev/null").catch(() => ({ stdout: "" }))
  ).stdout.trim();
