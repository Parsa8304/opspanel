...
export const GET = handler(async (req: NextRequest) => {
  ...
  const [publicIpRes, ipAddrRes, routeRes, resolvRes, hostnameRes] = await Promise.allSettled([
    ...
    hostExec("ip addr show 2>/dev/null"),
    hostExec("ip route 2>/dev/null"),
    hostReadFile("/etc/resolv.conf"),
    hostExec("hostname 2>/dev/null"),
  ]);
...
export const POST = handler(async (req: NextRequest) => {
  ...
  try {
    const { stdout, stderr } = await hostExec(cmd, 30000);
