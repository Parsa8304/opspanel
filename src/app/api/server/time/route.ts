...
export const GET = handler(async (req: NextRequest) => {
  ...
  const [tdRes, syncRes, confRes] = await Promise.allSettled([
    hostExec("timedatectl show --no-pager 2>/dev/null"),
    hostExec(
  ...
  try {
    ...
    if (stdout.trim() === "active") {
      ...
      const { stdout: c } = await hostExec(
        "systemctl is-active chronyd 2>/dev/null || true"
      );
      if (c.trim() === "active") {
        ntpService = "chrony";
      } else {
        const { stdout: n } = await hostExec(
...
export const POST = handler(async (req: NextRequest) => {
  ...
  switch (action) {
    case "set-timezone": {
      ...
      // Validate timezone
      const { stdout: tzList } = await hostExec(
      ...
      await hostExec(`timedatectl set-timezone ${timezone}`);
      await audit(
        u.id,
    ...
    case "enable-ntp":
      await hostExec("timedatectl set-ntp true");
      await audit(
        u.id,
      ...
      return json({ ok: true });
    case "disable-ntp":
      await hostExec("timedatectl set-ntp false");
      await audit(
        u.id,
    ...
    case "set-ntp-server": {
      ...
      await hostExec("systemctl restart systemd-timesyncd 2>/dev/null || true");
      await audit(
        u.id,
